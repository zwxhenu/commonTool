...
class Image{
    ...
    function thumb($image,$is_save=true,$suofang=0,$type='',$maxWidth=180,$maxHeight=180,$interlace=true){
        // 获取原图信息
       $info  = $this->getImageInfo($image);
         if($info !== false) {
            ...
            if ($suofang==1) {
				...
				$thumbname04 = substr_replace($image,"04.".$type,$length);		//小头像
				$imageFun($thumbImg,$thumbname01,100);
				$imageFun($thumbImg,$thumbname02,100);
				$imageFun($thumbImg,$thumbname03,100);
				$imageFun($thumbImg,$thumbname04,100);

                $thumbImg01 = imagecreatetruecolor(180,180);
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],180,180,$_POST['w'],$_POST['h']);

				$thumbImg02 = imagecreatetruecolor(90,90);
                imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],90,90,$_POST['w'],$_POST['h']);
                $thumbImg03 = imagecreatetruecolor(45,45);
                imagecopyresampled($thumbImg03,$thumbImg,0,0,$_POST['x'],$_POST['y'],45,45,$_POST['w'],$_POST['h']);
                $thumbImg04 = imagecreatetruecolor(30,30);
                imagecopyresampled($thumbImg04,$thumbImg,0,0,$_POST['x'],$_POST['y'],30,30,$_POST['w'],$_POST['h']);

				$imageFun($thumbImg01,$thumbname01,100);
				$imageFun($thumbImg02,$thumbname02,100);
				$imageFun($thumbImg03,$thumbname03,100);
				$imageFun($thumbImg04,$thumbname04,100);
//				unlink($image);
				imagedestroy($thumbImg01);
				imagedestroy($thumbImg02);
				imagedestroy($thumbImg03);
				imagedestroy($thumbImg04);
				imagedestroy($thumbImg);
				imagedestroy($srcImg);
				return array('180' => $thumbname01 , '90' => $thumbname02, '45' => $thumbname03, '30' => $thumbname04);	//返回包含大小头像路径的数组
            }else{
